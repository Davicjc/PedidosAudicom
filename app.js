...
function getSig(id) {
  ...
  for (let i = 3; i < pixels.length; i += 4) { if (pixels[i] > 0) return s.canvas.toDataURL(); }
  return null;
}
...
// ── ADM LOGIN ─────────────────────────────────────────
function _salvarLocal(nome, senha) { try { localStorage.setItem('aud_adm', JSON.stringify({ nome, senha })); } catch (_) {} }
function _limparLocal()           { try { localStorage.removeItem('aud_adm'); } catch (_) {} }
function _lerLocal()              { try { return JSON.parse(localStorage.getItem('aud_adm')); } catch (_) { return null; } }
...
async function abrirAdmLogin() {
  ...
  // Se já criamos um admin antes, pula a consulta ao Firestore
  ...
  if (!colAdm) {
    console.warn('[ADM] colAdm não inicializado — Firebase pode não ter carregado.');
    _admState('adm-login-normal');
    return;
  }
  try {
    // source:'server' evita retorno de cache vazio (offline persistence)
    ...
    console.error('[ADM] Erro ao verificar usuários:', e.code, e.message);
    if (e.code === 'permission-denied') {
      toast('Sem permissão de leitura. Verifique as regras do Firestore.', 'err');
    }
    // Em caso de falha (offline, regra, etc.) assume que admin existe e mostra login
    _admState('adm-login-normal');
  }
}
...
async function criarAdmMaster() {
  ...
  if (senha.length < 4) { erroEl.querySelector('span').textContent = 'Mínimo 4 caracteres.'; erroEl.classList.remove('hidden'); return; }
  ...
  try {
    ...
    erroEl.querySelector('span').textContent = 'Erro: ' + err.message;
  ...
  try {
    ...
    if (match) {
      if (document.getElementById('adm-salvar-check')?.checked) {
        _salvarLocal(document.getElementById('adm-nome').value.trim(), senha);
      } else _limparLocal();
      st.adm = { nome: match.data().nome, id: match.id, master: match.data().master === true };
      iniciarPainel();
    } else {
      erroEl.querySelector('span').textContent = 'Nome ou senha incorretos.';
      erroEl.classList.remove('hidden');
...
async function criarNovoUsuario() {
  ...
  } catch (e) { erroEl.querySelector('span').textContent = 'Erro: ' + e.message; erroEl.classList.remove('hidden'); }
}
...
async function salvarUsuario(id) {
  ...
  if (nova && nova.length < 4) { if (erroEl) { erroEl.textContent = 'Mínimo 4 caracteres.'; erroEl.classList.remove('hidden'); } return; }
